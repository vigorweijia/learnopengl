...
void hello_coordinate() {
	...
	while (!glfwWindowShouldClose(window)) {
		...
		unsigned int projectionLoc = glGetUniformLocation(shaderManager.ID, "projection");
...
void hello_coordinate_multicube() {
	/*-------------  glfw initialization   --------------*/
	glfwInit();
	glfwWindowHint(GLFW_CONTEXT_VERSION_MAJOR, 3);
	glfwWindowHint(GLFW_CONTEXT_VERSION_MINOR, 3);
	...
	while (!glfwWindowShouldClose(window)) {
		...
		glm::mat4 view(1.0f);
		glm::mat4 projection(1.0f);
		view = glm::translate(view, glm::vec3(0.0f, 0.0f, -3.0f)); // z变小，远离场景
		projection = glm::perspective(glm::radians(45.0f), 800.0f / 600.0f, 0.1f, 100.0f);
		unsigned int viewLoc = glGetUniformLocation(shaderManager.ID, "view");
		glUniformMatrix4fv(viewLoc, 1, GL_FALSE, glm::value_ptr(view));
		unsigned int projectionLoc = glGetUniformLocation(shaderManager.ID, "projection");
		...
		for (int i = 0; i < 10; i++) {
			...
			float angle_ratio = 1.0f;
			if (i % 3 == 0)
				angle_ratio = (float)glfwGetTime();
			model = glm::rotate(model, angle_ratio * glm::radians(50.0f) + glm::radians(20.0f * i), glm::vec3(1.0f, 0.3f, 0.5f));
			glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
			glDrawElements(GL_TRIANGLES, 36, GL_UNSIGNED_INT, 0);
		}
